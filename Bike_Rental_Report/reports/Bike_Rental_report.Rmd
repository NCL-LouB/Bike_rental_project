...
* Update the date variables (yr, month, weekday and days_since_2011)
+ The days_since_2011 variable is helpful as it indicates the date for each observation. It would be easier to navigate the data if proper date fields were created. 
+ Create date column, use the days_since_2011 variable to quality check the new date variable. Eg. observation 31 of days_since_2011 is 30 but this equates to 31 January 2011.
+ Introduce the lubridate package to create new date variables, just.day, just.dayofweek, just.dayofweek2 (character labels) , just.month, just.month2 (character labels) and year
+ Remove the original date columns, which are no longer needed (yr, mnth, weekday)
* Convert categorical character variables to factors (season, holiday, workingday and weathersit)
* Rename variables for clarity and to create naming convention ('cnt' to 'rental.count', 'weathersit' to 'weather.category', 'temp' to 'temperature, 'hum' to 'humidity', 'windspeed' to 'wind.speed', 'workingday' to 'working.day' and 'days_since_2011' to 'days.since.2011')
* Reorder variables so the response variable is listed first
* Round the temperature, wind speed and humidity columns so they become integers
* Create a new data frame for regression
+ Substitute numbers for the categorical, factor values (season, weather.category, holiday and working.day)
